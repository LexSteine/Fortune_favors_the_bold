def MassVote(N, Votes):
    if CheckCountOfWinners(Votes, FindMaxNumber(Votes)) and float(FindPercent(Votes, FindMaxNumber(Votes))) > 50.000:
        return f"majority winner {Votes.index(FindMaxNumber(Votes)) + 1}"
    elif CheckCountOfWinners(Votes, FindMaxNumber(Votes)) and float(FindPercent(Votes, FindMaxNumber(Votes))) <= 50.000:
